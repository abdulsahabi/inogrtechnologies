...
export default function PortfolioClient({ initialProjects }) {
  ...
  const handleFilterChange = (cat) => {
    ...
    // Update URL
    if (cat === "All") router.push("/portfolio", { scroll: false });
    else
  ...
  const scrollToGrid = () => {
    if (gridTopRef.current) {
      const y =
        gridTopRef.current.getBoundingClientRect().top + window.scrollY - 180;
                  ...
                  >
                    ...
                    >
                      <div className="relative overflow-hidden rounded-[32px] bg-gray-50 dark:bg-[#0a0a0a] border border-gray-200 dark:border-zinc-800 shadow-sm hover:shadow-xl transition-all duration-500">
                        {/* Image */}
                        <div
                          className={`relative w-full aspect-square md:aspect-[16/10] overflow-hidden ${
                            project.image.startsWith("bg-") ? project.image : ""
                          }`}
                        >
                          {/* If Real Image, Render It */}
